    ...
    <div class="mTop">
      ...
      <div class="item1">
        <van-row>
          ...
          <van-col span="12" @click.native="urlRecord()" >
      ...
      <div style="margin-top:10px">
        <van-row gutter="1">
          ...
          <van-col span="8">
            <div class="myList" @click="goUrl('/user/myIntegral')">
        ...
        <van-row gutter="1">
          <van-col span="8">
            <div class="myList" @click="goUrl('/home/activity')">
              <van-icon name="hot" style="font-size:1.5rem;color:#FF8200" />
              <div>热门活动</div>
            </div>
          </van-col>
          <van-col span="8">
            <div class="myList" @click="goTel()">
              <svg-icon :icon-class="'kefu'" style="color:#32c8fa"></svg-icon>
              <div>客服中心</div>
            </div>
          </van-col>
          <van-col span="8">
            <div class="myList" @click="goUrl('/user/feedback')">
...
<script>
import { CellGroup, Cell, Button, Row, Col, Popup, Icon } from 'vant'
...
export default {
  ...
  methods: {
    urlRecord() {
      this.$router.push({ path: '/user/myIntegral/integralRecord' })
    },
    onClickRight() {
    ...
    jifenAdd() {
      if (this.user.id) {
        integralInsert({ ownerId: this.user.id, taskName: '签到', type: 0 }).then((data) => { // 获取数据
          ...
          this.show = true
          getUser().then((datad) => {
            this.integralNum = datad.new.totalIntegral
