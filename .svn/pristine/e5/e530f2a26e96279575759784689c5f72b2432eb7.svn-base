<template>
  <div>
     <van-nav-bar
      ...
      :z-index='10'
    />
    <van-cell-group class="mTop" style="z-index:10" >
      <van-cell  :title="locationCity.city" value="当前定位城市" icon="location" v-show="locationCity&&locationCity.id"   @click="selectCity(locationCity)" />
    </van-cell-group>
...
export default {
  ...
  created() {
    ...
    const { locationCity } = localStorageTool.getLocalStorage('locationCity')
    if (locationCity) { // 地区有数据 赋值
      this.locationCity = JSON.parse(locationCity)
    }
  },
  computed: {
    data() {
