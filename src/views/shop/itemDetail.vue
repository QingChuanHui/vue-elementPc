<template>
  <div class="home" >
    ...
    <div class="item-top">
      <span class="item-price">￥{{itemDetail.itemPrice}}</span>
      <span class="item-title">{{itemDetail.itemName}}</span>
    </div>
    <div class="note">
    ...
    <div class="form_wrap" style="margin-bottom:60px">
        ...
        <div class="item_list">
          <ul>
            ...
            <li>
              <span class="order-label">适用车型</span>
              <p v-for="(item,index) in itemDetail.basePartCarTypeList" :key="index">{{item.carTypeName}}</p>
            </li>
            <li>
              <span class="order-label">项目商品</span>
              <span v-for="(item1,index) in itemDetail.baseCompanyItemPartList" :key="index">{{item1.partName}}</span>&nbsp;&nbsp;
            </li>
            <li>
              <span class="order-label">服务时长</span>
              {{itemDetail.itemServiceTime}}
            </li>
            <li>
              <span class="order-label">建议周期</span>
              {{itemDetail.suggestCycle}}
            </li>
          </ul>
        </div>
      ...
      <div class="page-bottom" >
        <van-button type="primary" bottom-action @click="goCharge(itemDetail.itemCode)">立即购买</van-button>
    </div>
  </div>
</template>
<script>
