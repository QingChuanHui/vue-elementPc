<template>
  <div>
    ...
    <div class="mTop">
        ...
        <div class="bill_store">
          <div class="store-detail">
              ...
              <div>
                ...
                <span v-show="$route.query.itemCode">{{itemDetail.itemName}}</span>
                <span v-show="$route.query.setmealId">{{mealDetail.name}}</span>
              </div>
          </div>
        </div>
      ...
      <div class="form_wrap" v-show="!$route.query.setmealId">
    ...
    <div class="page-bottom">
        <div class="xi-pay">
          ￥
          <span class="xi-pay-money" v-show="$route.query.itemCode">{{itemDetail.itemPrice}}</span>
          <span class="xi-pay-money" v-show="$route.query.setmealId">{{mealDetail.price}}</span>
        </div>
...
import { itemDetail, getShopDetail, mealDetail } from '@/api/shop'
...
export default {
  ...
  methods: {
    ...
    goPay() {
      if (this.$route.query.itemCode) {
        this.createbill = { ownerId: this.user.id, carCode: this.carCode, companyId: this.$route.query.id, serviceType: this.itemDetail.type, couponId: this.couponId, mgrBillDetailItemList: [{ itemId: this.itemDetail.id }], contectPerson: this.username, contectPersonPhone: this.phone, appointmentTime: this.time, contectPersonSex: this.sex, isItem: 1 }
      }
      if (this.$route.query.setmealId) {
        this.createbill = { ownerId: this.user.id, carCode: this.carCode, companyId: this.$route.query.id, mgrBillDetailSetmealList: [{ setmealId: this.mealDetail.id }], contectPerson: this.username, contectPersonPhone: this.phone, appointmentTime: this.time, contectPersonSex: this.sex, isItem: 0 }
      }
