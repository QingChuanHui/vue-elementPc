...
export default {
  ...
  mounted() {
    const { userInfo } = localStorageTool.getLocalStorage('userInfo')
    if (userInfo) { // 获取用户信息、
  ...
  beforeRouteEnter(to, from, next) {
    to.query.isUpdate = 1 // 1刷新数据
    if (from.path === '/user/myOrder/detail') {
      to.query.isUpdate = 0 // 详细返回
    }
    next()
  },
  methods: {
