...
export default {
  ...
  mounted() {
    if (this.$route.params.integralBill) {
      ...
      this.integralOK.couponType = this.$route.params.couponType // 优惠券类型
    }
  },
  methods: {
    onClickLeft() {
    ...
    sureIngegrall() {
      if (this.integralOK.couponType === 0) { // 优惠券是洗车
        this.$router.replace({ path: '/shop/washCar' })
      } else { // 保养
        this.$router.replace({ path: '/shop/companyList?type=' + this.integralOK.couponType })
      }
    }
  },
  components: {
